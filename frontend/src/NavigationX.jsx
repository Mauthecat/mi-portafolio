        ...
        {showFinale && (
          ...
          >
            ...
            >
              ...
              >
                ¡Trabajemos Juntos!import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { FaArrowLeft, FaHandshake } from 'react-icons/fa'; 
import PerfilContent from './PerfilContent';
import ContactoContent from './ContactoContent';
    ...
    <div style={{
      ...
      <motion.div 
        ...
        style={{
          display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr',
          gap: '12px',
          // Usamos 75vh para que nunca desborde verticalmente
          width: '75vh', height: '75vh', 
          maxWidth: '90vw', maxHeight: '90vh', // Seguros para móvil
          zIndex: 10, pointerEvents: selectedId ? 'none' : 'auto'
        }}
      >
        {sections.map((section) => (
          ...
          >
            ...
            >
              <motion.h2 layoutId={`title-${section.id}`} style={{ margin: 0, color: '#fff', fontSize: 'clamp(1rem, 3.5vh, 2rem)', fontWeight: '900', lineHeight: 1 }}>
                {section.title}
              </motion.h2>
              <motion.p layoutId={`subtitle-${section.id}`} style={{ margin: 0, color: 'rgba(255,255,255,0.9)', fontSize: 'clamp(0.6rem, 1.8vh, 0.9rem)' }}>
                {section.subtitle}
              </motion.p>
            </motion.div>
          </motion.div>
        ...
        {selectedId && (
          <motion.div
            ...
            style={{ 
              position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', 
              background: sections.find(s => s.id === selectedId).color, 
              zIndex: 50, display: 'flex', flexDirection: 'column', padding: 'clamp(10px, 3vw, 30px)',
              overflowY: 'hidden' 
            }}
          >
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ width: '100%', height: '100%', maxWidth: '1400px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  <div>
                    <motion.h1 layoutId={`title-${selectedId}`} style={{ fontSize: 'clamp(1.4rem, 5vh, 2.5rem)', color: 'white', margin: 0 }}>{sections.find(s => s.id === selectedId).title}</motion.h1>
                  </div>
                  <button onClick={() => setSelectedId(null)} style={{ background: 'rgba(0,0,0,0.5)', color: 'white', border: 'none', borderRadius: '50px', padding: '8px 15px', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}><FaArrowLeft /> VOLVER</button>
                </div>
        ...
        {showFinale && (
          ...
          >
            <motion.div
              ...
              style={{
                background: 'linear-gradient(135deg, #1a2a6c, #b21f1f, #fdbb2d)',
                padding: '30px', borderRadius: '20px', maxWidth: '500px', width: '90%', textAlign: 'center',
                color: 'white', border: '1px solid rgba(255,255,255,0.2)'
              }}
            >
              <div style={{ fontSize: '3rem', marginBottom: '10px' }}>🚀</div>
